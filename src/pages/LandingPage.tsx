      ...
      <div className="flex justify-center gap-2 mt-6">
        {testimonials.map((_, index) => (
          <button
            ...
            className={`w-2.5 h-2.5 rounded-full transition-colors ${current === index ? 'bg-primary' : 'bg-muted-foreground/30'}`}
          />
        ))}
      </div>
    </div>
...
function SimpleLogo({ size = "md" }: { size?: "sm" | "md" | "lg" }) {
  const sizes = {
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex justify-center mt-6">
              <Button
                size="lg"
                className="gap-2 text-base px-8 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold"
              ...
              >
                ...
                <CardHeader className="text-center pb-2">
                  ...
                  {plan.savings && (
                    <Badge
                      variant="secondary"
                      className={`${plan.id === 'annual' ? 'bg-destructive/10 text-destructive border-destructive/20' : 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100'}`}
                    >
                      {plan.savings}
                    </Badge>
                  )}
